...
def interpolate_cross_section(data, energies):
    interpolation_function = interp1d(
        data["Energy"], data["SigmaRho"], kind="linear", fill_value="extrapolate"
    )
...
def model_scattered_spectrum_with_density(
    ffast_data_dict, incident_spectrum, energies, element_properties
):
    ...
    for element, properties in element_properties.items():
        ...
        if element in ffast_data_dict:
            ...
            element_scattered_spectrum = (
                number_density * sigma_rho_interpolated * incident_spectrum
            )
...
def main():
    ...
    if incident_spectrum is not None:
        ...
        result_df = pd.DataFrame(
            {"Energy": energies, "Total Scattered Spectrum": total_scattered_spectrum}
        )
