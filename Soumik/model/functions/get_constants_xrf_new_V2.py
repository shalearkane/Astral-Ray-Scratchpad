...
def get_constants_xrf(
    energy: list, at_no: list, weight: list, xrf_lines: Xrf_Lines
) -> Const_Xrf:
        ...
        for j in range(0, n_lines):
            ...
            if (line_energy > 0) and (rad_rate > 0):
                for k in range(0, no_elements):
                    ...
                    func_interp = interp1d(x_interp, y_interp, fill_value="extrapolate") # type: ignore
                    muelement_echarline = func_interp(line_energy)
                    musampletotal_echarline[i, j] = (
    ...
    for i in range(0, no_elements):
        for j in range(0, n_lines):
            ...
            if (line_energy > 0) and (
                rad_rate > 0
            ):  # ; This is to ensure that only proper XRF lines are taken
                for k in range(0, no_elements):
                    tmp3 = np.where(xrf_lines.energy_nist[k, :] != 0)
                    x_interp = (xrf_lines.energy_nist[k, tmp3])[0, :]
                    y_interp = (xrf_lines.totalcs_nist[k, tmp3])[0, :]
                    ...
                    musampletotal_eincident[i, j, :] = (
                        musampletotal_eincident[i, j, :]
                        + weight[k] * muelement_eincident
                    )
    ...
    for i in range(0, no_elements):
        for j in range(0, n_lines):
            ...
            if (line_energy > 0) and (
                ...
                tmp4 = np.where(energy < edge_energy)
                if n_elements(tmp4) != 0:
                    muelementphoto_eincident[i, j, tmp4] = (
                        0.0  # ; Setting values less than the edge energy as 0
                    )

    # ; Creating a structure to return the data
    return Const_Xrf(
        musampletotal_echarline, musampletotal_eincident, muelementphoto_eincident
    )
