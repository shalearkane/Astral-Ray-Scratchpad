...
def readcol(filename: str, format: Optional[str] = None) -> tuple:
    if format == None:
        rowformat = []
    else:
...
def SortVectors(TupleOfArrays: tuple, Reverse: bool = False) -> tuple:
    ...
    for w in TupleOfLists:
        if w.__len__() != length:
            return ListOfTuples  # type: ignore
...
def array_indices_custom(NpArray, *location) -> list:
    ...
    return np.array(ReturnValue)  # type: ignore
...
class Const_Xrf:
    def __init__(
        self, Musampletotal_echarline, Musampletotal_eincident, Muelementphoto_eincident
    ) -> None:
...
class Constant_scat:
    def __init__(
        self, energy_compton, mu_coh, mu_incoh, mu_photo, salpha_lamda
    ) -> None:
