...
def get_xrf_lines(
    ...
    for i in range(0, no_elements):
        ...
        filename = (
            ...
            + (ele_list[tmp1])[0]
            + ".txt"
        )  # ; Getting the attenuation coefficients from FFAST database
        (column1, column2, column3, column4, column5, column6, column7, column8) = (
            readcol(filename, format="D,F,F,F,F,F,F,F")
        )
        ...
        edgeenergy[i, 0:2] = xraylib.EdgeEnergy(
            at_no[i], k_shell
        )  # ; edge energy is defined for shells so it will be same for kalpha and kbeta
        ...
        fluoryield[i, 0:2] = xraylib.FluorYield(
            at_no[i], k_shell
        )  # ; fluorescent yield is defined for shells so it will be same for kalpha and kbeta
        try:
        ...
        jumpfactor[i, 0:2] = xraylib.JumpFactor(
            at_no[i], k_shell
        )  # ; jump factor is defined for shells so it will be same for kalpha and kbeta
        try:
        ...
        if n_elements(allowed_lines_index_kbeta) != 0:  # then begin
            lineenergy[i, 0] = total(
                radrate_kbeta[allowed_lines_index_kbeta]
                * lineenergy_kbeta[allowed_lines_index_kbeta]
            ) / total(
                radrate_kbeta[allowed_lines_index_kbeta]
            )  # ; weighted average of kbeta energies
        ...
        if n_elements(allowed_lines_index_kalpha) != 0:  # then begin
            lineenergy[i, 1] = total(
                radrate_kalpha[allowed_lines_index_kalpha]
                * lineenergy_kalpha[allowed_lines_index_kalpha]
            ) / total(
                radrate_kalpha[allowed_lines_index_kalpha]
            )  # ; weighted average of kalpha energies
        ...
        if n_elements(allowed_lines_index_l1) != 0:  # then begin
            lineenergy[i, 2] = total(
                radrate_l1[allowed_lines_index_l1]
                * lineenergy_l1[allowed_lines_index_l1]
            ) / total(
                radrate_l1[allowed_lines_index_l1]
            )  # ; weighted average of l1 energies
        ...
        if n_elements(allowed_lines_index_l2) != 0:  # then begin
            lineenergy[i, 3] = total(
                radrate_l2[allowed_lines_index_l2]
                * lineenergy_l2[allowed_lines_index_l2]
            ) / total(
                radrate_l2[allowed_lines_index_l2]
            )  # ; weighted average of l2 energies
        ...
        if n_elements(allowed_lines_index_l3[0]) != 0:  # then begin
            lineenergy[i, 4] = total(
                radrate_l3[allowed_lines_index_l3]
                * lineenergy_l3[allowed_lines_index_l3]
            ) / total(
                radrate_l3[allowed_lines_index_l3]
            )  # ; weighted average of l3 energies
